...
int dx = 0, dy = 0;
int showcross = 1;
int shape = 0;//0 = rectangle, 1 = triangle
...
COLORREF const RED = RGB(255, 0, 0);//red
COLORREF const BLUE = RGB(0, 0, 255);//blue

COLORREF shapeColor = RED;
COLORREF crossColor = BLUE;
...
void draw_line(HDC DC, int x, int y, int a, int b, COLORREF color, int width);	//a draw line function - Edited for paint
void draw_rectangle(HDC DC, int x, int y, int w, int h, COLORREF color, int width);//Edited for paint
void draw_triangle(HDC DC, int x, int y, int w, int h, COLORREF color, int width);//Edited for paint
void draw_star(HDC DC, int x, int y, int w, int h, COLORREF color, int width);//Edited for paint

//void draw_line(HDC DC, int x, int y, int a, int b, COLORREF color, int width);//Added for painting
	...
	int       nCmdShow)
{
	...
	hMain = CreateWindow(MainWin, "CST F18 Homework 1", WS_OVERLAPPEDWINDOW | WS_VISIBLE, 200, 200, 1300, 900, NULL, NULL, hInst, NULL);
	if (hMain==0)	return 0;
...
{
	if (paintOn) {
		painting = true;//Added for painting
	}
	else {
		painting = false;
	}
...
BOOL OnCreate(HWND hwnd, CREATESTRUCT FAR* lpCreateStruct)
{
	...
	colors.push_back(RGB(255, 192, 0));		//Orange - added
	colors.push_back(RGB(255, 255, 0));		//Yellow - added
	colors.push_back(RGB(0, 255, 0));		//Green
	colors.push_back(RGB(0, 0, 255));		//Blue
	colors.push_back(RGB(64, 0, 128));		//Indigo - added
	colors.push_back(RGB(128, 0, 128));		//Violet - added
	colors.push_back(RGB(255, 255, 255));	//White
	colors.push_back(RGB(128, 128, 128));	//Gray - added
	///////////////////////////////////
...
void OnPaint(HWND hwnd)
	...
	for (int i = 1; i < paintedPixels.size(); ++i)
	{
		...
		draw_line(DC, paintedPixels[i - 1].x, paintedPixels[i - 1].y, paintedPixels[i].x, paintedPixels[i].y, paintedPixels[i].color, 5);
	}
	///////////////////////////////////
	...
	//Rectangle
	if (shape == 0) {
		draw_rectangle(DC, dx, dy, 300, 300, shapeColor, 5);//Edited for paint
	}

	//Triangle
	else if (shape == 1) {
		draw_triangle(DC, 300, 300, dx, dy, shapeColor, 5);//Edited for paint
	}
	//Star
	else if (shape == 2 && showcross == 1) {
		draw_star(DC, 300, 300, dx, dy, shapeColor, 5);//Edited for paint
	}

	//Cross
	if (showcross == 1)
	{
	draw_line(DC, dx - 10, dy, dx + 10, dy, crossColor, 5);//Edited for paint
	draw_line(DC, dx, dy - 10, dx, dy + 10, crossColor, 5);//Edited for paint
	}
...
void draw_line(HDC DC,int x, int y, int a, int b, COLORREF color, int width)//edited for paint (COLORREF color)
	{
	HPEN Stift = CreatePen(PS_SOLID, width, color);//edited for paint (color)
...
void draw_rectangle(HDC DC, int x, int y, int w, int h, COLORREF color, int lineWidth)//edited for paint (COLORREF color)
{
	draw_line(DC, x, y, w, y, color, lineWidth);//edited for paint ( color)
	draw_line(DC, w, y, w, h, color, lineWidth);//edited for paint ( color)
	draw_line(DC, w, h, x, h, color, lineWidth);//edited for paint ( color)
	draw_line(DC, x, h, x, y, color, lineWidth);//edited for paint ( color)
}

void draw_triangle(HDC DC, int x, int y, int a, int b, COLORREF color, int lineWidth)//edited for paint (COLORREF color)
{
	draw_line(DC, x, y, a, b, color, lineWidth);// /
	draw_line(DC, x, y, a, y, color, lineWidth);// _
	draw_line(DC, a, y, a, b, color, lineWidth);//  |
}

void draw_star(HDC DC, int x, int y, int a, int b, COLORREF color, int lineWidth)//edited for paint (COLORREF color)
{
	int lBottomX = x;
	int lBottomY = y;
	...
	draw_line(DC, x, y, topX, topY, color, lineWidth); // L1 
	draw_line(DC, rBottomX, rBottomY, topX, topY, color, lineWidth); // L2 
	draw_line(DC, lMiddleX, lMiddleY, rMiddleX, rMiddleY, color, lineWidth); // L3 
	draw_line(DC, lMiddleX, lMiddleY, rBottomX, rBottomY, color, lineWidth); // L4 
	draw_line(DC, rMiddleX, rMiddleY, lBottomX, lBottomY, color, lineWidth); // L5 
}
...
LRESULT CALLBACK WndProc(HWND hwnd, UINT message, WPARAM wParam, LPARAM lParam)
{
	...
	*/
		...
		HANDLE_MSG(hwnd, WM_RBUTTONDOWN, OnRBD);	//!@@ Added for paint @@!
