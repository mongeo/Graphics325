...
ID3D11Buffer*           g_pVertexBuffer3 = NULL; //our model
...
HRESULT InitDevice()
{
	...
	// Create vertex buffer, the hour glass
	SimpleVertex hourglass[numVertices];
	hourglass[0].Pos = XMFLOAT3(-0.5f, 0.5f, 0.5f);
	hourglass[1].Pos = XMFLOAT3(0.5f, 0.5f, 0.5f);
	hourglass[2].Pos = XMFLOAT3(0.0f, 0.0f, 0.5f);
	
	hourglass[3].Pos = XMFLOAT3(-0.5f, -0.5f, 0.5f);
	hourglass[4].Pos = XMFLOAT3(0.0f, 0.0f, 0.5f);
	hourglass[5].Pos = XMFLOAT3(0.5f, -0.5f, 0.5f);
	...
	InitData.pSysMem = hourglass;
	hr = g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pVertexBuffer);
	if (FAILED(hr))
		return hr;

	// Square1
	SimpleVertex square[numVertices];
	square[0].Pos = XMFLOAT3(-1.5f + dx, 0.5f, 0.5f);
	square[1].Pos = XMFLOAT3(-0.5f + dx, 0.0f, 0.5f);
	square[2].Pos = XMFLOAT3(-1.5f + dx, 0.0f, 0.5f);

	square[3].Pos = XMFLOAT3(-1.5f + dx, 0.5f, 0.5f);
	square[4].Pos = XMFLOAT3(-0.5f + dx, 0.5f, 0.5f);
	square[5].Pos = XMFLOAT3(-0.5f + dx, 0.0f, 0.5f);
	...
	// Square2
	SimpleVertex square2[numVertices];
	square2[0].Pos = XMFLOAT3(0.5f, 0.0f, 0.5f);
	square2[1].Pos = XMFLOAT3(1.0f, 0.0f, 0.5f);
	square2[2].Pos = XMFLOAT3(1.0f, -0.5f, 0.5f);

	square2[3].Pos = XMFLOAT3(0.5f, 0.0f, 0.5f);
	square2[4].Pos = XMFLOAT3(1.0f, -0.5f, 0.5f);
	square2[5].Pos = XMFLOAT3(0.5f, -0.5f, 0.5f);
	...
	bd.CPUAccessFlags = 0;
	ZeroMemory(&InitData, sizeof(InitData));
	InitData.pSysMem = square2;
	hr = g_pd3dDevice->CreateBuffer(&bd, &InitData, &g_pVertexBuffer3);
	if (FAILED(hr))
		return hr;
	...
	VsConstData.some_variable_c = -0.5;
	VsConstData.some_variable_d = -0.5;
...
void Render()
{
	...
	if (pressed)
	{
		VsConstData.some_variable_a = VsConstData.some_variable_b =
			VsConstData.some_variable_c = 1;
	}
	else
	{
		VsConstData.some_variable_a = 1;
		VsConstData.some_variable_b = 0;
	...
	*/
	...
	//VsConstData.some_variable_a = dx;
	...
	g_pImmediateContext->Draw(numVertices, 0);		//whatever you have set before as model (mesh, vertexbuffer) will be drawn here finally. First parameter is the vertex count, second the offset
	...
	//draw square2
	g_pImmediateContext->IASetVertexBuffers(0, 1, &g_pVertexBuffer3, &stride, &offset);
	g_pImmediateContext->Draw(6, 0);
